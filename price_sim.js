...
const dex_contract_source = fs.readFileSync(__dirname + '/bridgeContract.json', 'utf-8');
const bridge_contract_source = fs.readFileSync(__dirname + '/dexContract.json', 'utf-8');
const erc20_contract_source = fs.readFileSync(__dirname + '/erc20.json', 'utf-8');

let dex_on_bsc_testnet = new ethers.Contract('0x2e3bD9C42af595862FB99159b780d69380847ca2', dex_contract_source, provider_BSC)
let dex_on_rinkeby = new ethers.Contract('0xBd342437b0EC13DB9C70Bb45f1958362A7D9D56B', dex_contract_source, provider_RINK)

let bridge_on_bsc_testnet = new ethers.Contract('0x89262B7bd8244b01fbce9e1610bF1D9F5D97C877', bridge_contract_source, provider_BSC)
let bridge_on_rinkeby = new ethers.Contract('0x89262B7bd8244b01fbce9e1610bF1D9F5D97C877', bridge_contract_source, provider_RINK)

let erc20_blxm_on_bsc_testnet = new ethers.Contract('0x6703bB029a9B2d44f8e21Ec7f635C5A0b06743Fa', erc20_contract_source, provider_BSC)
let erc20_stable_on_bsc_testnet = new ethers.Contract('0x42db14a9f863f12a9265365c1901a36e58c862fc', erc20_contract_source, provider_BSC)

let erc20_blxm_on_rinkeby = new ethers.Contract('0xb5382dfba952a41a2f2d0b7313c3578b83d32be0', erc20_contract_source, provider_RINK)
let erc20_stable_on_rinkeby= new ethers.Contract('0x22853d67597b47Fb11c7569Ab507530216cA56a4', erc20_contract_source, provider_RINK)
...
async function swap(amount, direction) {
  ...
  async function findWindow() {
    ...
    async function findAllValues (direction) {
      ...
      console.log("currnet percentage on rinkeby (blxm, stable_coin): " + val)
    ...
    if(direction == 1) {
      if(findAllValues(1)) { // If blxm is expensive on the target network
        // perform swap
    ...
    if(direction == 2) {
      if(findAllValues(2)) { // If blxm is expensive on the target network
        // perform swap
  ...
  function percent_of_tokens(a,b) {
    var c = a+b;
    per_a = (a * 100) / c;
    per_b = (b * 100) / c;
